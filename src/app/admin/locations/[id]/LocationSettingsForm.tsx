...
export default function LocationSettingsForm({ location }: { location: any }) {
  ...
  const [basePrice, setBasePrice] = useState(location.basePrice.toString());
  const [minPriceFloor, setMinPriceFloor] = useState(
    location.minPriceFloor.toString(),
  );
      ...
      <div className="flex justify-between items-center border-b border-black/5 dark:border-white/5 pb-4">
        ...
        <button
          ...
          className={cn(
            ...
            "w-auto px-6 py-2 text-sm flex items-center gap-2 disabled:opacity-50 disabled:grayscale",
          )}
        >
      ...
      <div className="grid grid-cols-2 gap-6">
        ...
        <div className="space-y-2">
          <div className="flex justify-between items-center px-1">
            ...
            {floorError && (
              <span className="text-[10px]  uppercase text-red-500 animate-pulse">
                {floorError}
              </span>
            )}
          </div>
          ...
          <div className="flex gap-2 h-12">
            {[
              ...
              <label key={day.val} className="flex-1 cursor-pointer group">
                ...
                />
                <div className="w-full h-full flex items-center justify-center rounded-xl bg-brand-black/5 dark:bg-brand-latte/5 peer-checked:bg-brand-blue peer-checked:text-brand-latte transition-all text-xs  uppercase tracking-widest border border-transparent peer-checked:border-brand-blue shadow-sm">
                  {day.label}
                </div>
              </label>
            ))}
