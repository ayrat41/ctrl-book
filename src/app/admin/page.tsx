...
export default async function AdminDashboard() {
  ...
  const [bookings, totalRevenue, totalStudios, recentBookings] = await Promise.all([
  ...
  return (
    <div className="space-y-8 max-w-6xl mx-auto">
      <div className="flex flex-col gap-2">
        ...
        <p className="text-neutral-500 font-medium">Here's what's happening with your studio today.</p>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="p-6 rounded-2xl bg-white dark:bg-[#111] border border-black/5 dark:border-white/5 shadow-sm">
           <div className="flex items-center justify-between mb-4">
              <span className="text-sm font-semibold opacity-60">Total Revenue</span>
              <div className="w-8 h-8 rounded-full bg-brand-blue/10 text-brand-blue flex items-center justify-center">
                 <DollarSign className="w-4 h-4" />
              </div>
           </div>
           <div className="text-3xl font-black tracking-tighter">${revenue.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</div>
        </div>

        {/* Metric 2 */}
        <div className="p-6 rounded-2xl bg-white dark:bg-[#111] border border-black/5 dark:border-white/5 shadow-sm">
           <div className="flex items-center justify-between mb-4">
              <span className="text-sm font-semibold opacity-60">Total Bookings</span>
              <div className="w-8 h-8 rounded-full bg-fuchsia-500/10 text-fuchsia-500 flex items-center justify-center">
        ...
        <div className="p-6 rounded-2xl bg-white dark:bg-[#111] border border-black/5 dark:border-white/5 shadow-sm">
           ...
           <div className="text-3xl font-black tracking-tighter">{totalStudios} <span className="text-base font-medium opacity-50">studios</span></div>
        </div>
           ...
           <table className="w-full text-left text-sm">
             ...
             <tbody className="divide-y divide-black/5 dark:divide-white/5">
                {recentBookings.length === 0 ? (
                  <tr>
                    <td colSpan={5} className="px-6 py-12 text-center opacity-50 font-medium italic">No recent bookings found.</td>
                  </tr>
                ) : (
                  recentBookings.map((b) => (
                    <tr key={b.id} className="hover:bg-brand-black/[0.02] dark:hover:bg-white/[0.02] transition-colors">
                       <td className="px-6 py-4 font-medium">{b.customer.fullName}</td>
