...
export default function BookingsClient({
  ...
  const handleRefund = async (id: string) => {
    if (
      !window.confirm(
        "Are you sure you want to trigger a Stripe refund? This cannot be undone.",
      )
    )
      return;
  ...
  const getStatusIcon = (status: string) => {
    switch (status) {
      ...
      case "refunded":
        return <RefreshCcw className="w-3 h-3 mr-1" />;
      default:
        return null;
    }
  };
        ...
        <div className="flex items-center gap-2 bg-white dark:bg-brand-latte/5 p-1.5 rounded-2xl border border-black/5 dark:border-white/10 shadow-sm">
          {["all", "confirmed", "pending", "cancelled"].map((status) => (
            <button
              ...
              className={cn(
                "px-4 py-2 rounded-xl text-xs  uppercase tracking-widest transition-all",
                statusFilter === status
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left text-sm">
            <thead className="bg-black/5 dark:bg-white/5 border-b border-black/5 dark:border-white/10">
              <tr>
                <th className="px-6 py-5  text-[10px] uppercase tracking-[0.2em] opacity-50">
                  Customer
                </th>
                <th className="px-6 py-5  text-[10px] uppercase tracking-[0.2em] opacity-50">
                  Session Details
                ...
                <th className="px-6 py-5  text-[10px] uppercase tracking-[0.2em] opacity-50 text-right">
                  Revenue
                </th>
                ...
                filteredBookings.map((b) => (
                  ...
                  >
                    ...
                    <td className="px-6 py-5">
                      <span
                        className={cn(
                          "inline-flex items-center px-3 py-1.5 rounded-xl text-[10px]  uppercase tracking-widest",
              ...
              <div className="p-8 space-y-10">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                  ...
                  <div className="space-y-4">
                    <h3 className="flex items-center gap-2 text-xs  uppercase tracking-widest opacity-40">
                    ...
                    <div className="bg-black/5 dark:bg-white/5 p-5 rounded-2xl space-y-3">
                      <div>
                        ...
                        <p className="text-xs opacity-60">
                          {selectedBooking.studio.location.name}
                        </p>
                      </div>
                      <div>
                ...
                {selectedBooking.addOns.length > 0 && (
                  <div className="space-y-4">
                    <h3 className="text-xs  uppercase tracking-widest opacity-40">
                      Active Add-ons
                    </h3>
                ...
                <div className="pt-10 flex flex-col sm:flex-row gap-3 border-t border-black/5 dark:border-white/10">
                  {selectedBooking.status !== "cancelled" &&
                    selectedBooking.status !== "refunded" && (
                      <button
                        ...
                        className="flex-1 flex items-center justify-center gap-2 py-4 bg-red-500 text-white  text-xs uppercase tracking-widest rounded-2xl hover:bg-red-600 transition-all shadow-lg disabled:opacity-50"
                      >
                  ...
                  {selectedBooking.stripePaymentId &&
                    selectedBooking.status !== "refunded" && (
                      <button
                        ...
                        disabled={isProcessing}
                        className="flex-1 flex items-center justify-center gap-2 py-4 border-2 border-purple-500/50 text-purple-600 dark:text-purple-400  text-xs uppercase tracking-widest rounded-2xl hover:bg-purple-500/10 transition-all disabled:opacity-50"
                      >
                  ...
                  {selectedBooking.status === "pending" && (
                    <button
                      ...
                      disabled={isProcessing}
                      className="flex-1 flex items-center justify-center gap-2 py-4 bg-green-500 text-white  text-xs uppercase tracking-widest rounded-2xl hover:bg-green-600 transition-all shadow-lg disabled:opacity-50"
                    >
